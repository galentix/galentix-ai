...
import { FileText, Globe, X, ExternalLink, ChevronUp, ChevronDown } from 'lucide-react';
...
export default function SourcesPanel({ sources, webResults, onClose }: SourcesPanelProps) {
  ...
  const totalCount = sources.length + webResults.length;

  return (
    <>
      {/* Desktop: side panel */}
      <div className="hidden md:flex w-80 border-s border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-y-auto flex-col">
        ...
        <div className="p-4 space-y-4 flex-1 overflow-y-auto">
          <SourcesList sources={sources} webResults={webResults} />
        </div>
      </div>
...
function SourcesList({ sources, webResults }: { sources: Source[]; webResults: WebSearchResult[] }) {
  return (
    <>
      {/* Document sources */}
      {sources.length > 0 && (
                ...
                {source.similarity !== undefined && (
                  <div className="mt-2">
                    ...
                    <div className="h-1.5 bg-gray-200 dark:bg-slate-600 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-galentix-500 rounded-full"
          ...
          <div className="space-y-2">
            {webResults.map((result, index) => (
              ...
              >
                ...
                <p className="text-xs text-galentix-600 mt-1 truncate">
                  {result.source}
                </p>
              </a>
            ))}
